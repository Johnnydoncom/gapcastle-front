            ...
            render={({ field: formField, fieldState }) => (
              <div className={`space-y-2 ${isHidden ? 'hidden' : ''}`}>
                ...
                {field.type === "plan_grid" && (() => {
                  ...
                  // First Central tags each report consumer or commercial and the
                  // entity toggle picks between them. CRC has no such split and
                  // tags nothing, so an equality test drops its whole catalogue.
                  // Only a product that carries a tag can be excluded by it —
                  // an untagged one belongs to a bureau that does not use them.
                  if (category === "credit_check" && values.requestType) {
                    visibleProducts = products.filter(
                      p => !p.metadata?.request_type || p.metadata.request_type === values.requestType
                    );
                  }
